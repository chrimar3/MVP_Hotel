...
class HybridGenerator {
    ...
    async initialize() {
        ...
        console.log('HybridGenerator initialized:', this.availability);
    }
        ...
        try {
            ...
            if (this.availability.openai) {
                try {
                    ...
                    return this.formatResponse(result.text, 'openai', performance.now() - startTime, requestId);
                } catch (error) {
            ...
            return this.formatResponse(templateResult, 'template', performance.now() - startTime, requestId);
    ...
    async checkAvailability() {
        ...
        // Check OpenAI
        if (this.config.openai.key && this.config.openai.key !== 'sk-YOUR-KEY-HERE') {
            try {
        ...
        // Check Groq
        if (this.config.groq.key && this.config.groq.key !== 'gsk_YOUR-KEY-HERE') {
            try {
                // Quick validation without actual API call
    ...
    saveMetrics() {
        if (typeof localStorage !== 'undefined') {
            try {
                ...
                localStorage.setItem('hybrid_cost', JSON.stringify({
...
class TemplateGenerator {
    ...
    highlightsToText(highlights, minimal = false) {
        ...
        if (sentences.length === 1) {
            ...
            return sentences[0].charAt(0).toUpperCase() + sentences[0].slice(1) + ' and ' + sentences[1] + '.';
        } else {
            const last = sentences.pop();
            return sentences.join(', ').charAt(0).toUpperCase() + sentences.join(', ').slice(1) + ', and ' + last + '.';
        }
    }
}
